...
export async function POST(request: NextRequest) {
  try {
    ...
    // verify if users are uploading to their own account
    ...
    const fileExtension = originalFilename.split(".").pop() || ""; // pop remove the last element and return it
    const uniqueFilename = `${uuidv4()}.${fileExtension}`;

    // upload file into imagekit
